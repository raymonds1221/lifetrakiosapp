...
- (void) scrollCollectionViewToSelectedDate {
    if (self.selectedIndexPath) {
        NSInteger lastItem = [self collectionView:self.collectionView numberOfItemsInSection:self.selectedIndexPath.section];
        NSIndexPath *indexPath = [NSIndexPath indexPathForItem:lastItem - 1 inSection:self.selectedIndexPath.section];
...
- (DFDatePickerDayCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
	...
	cell.selected = [[SFACalendarView activeCalendarView] isCellDateSelected:cellDate];
	cell.hasData = [self.datesWithData containsObject:cellDate];
    
    if (cell.selected && cell.enabled)
    {
