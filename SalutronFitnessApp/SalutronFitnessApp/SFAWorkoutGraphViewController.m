//
//  SFAWorkoutGraphViewController.m
//  SalutronFitnessApp
//
...
@interface SFAWorkoutGraphViewController ()<SFABarPlotDelegate,UIScrollViewDelegate>
...
@implementation SFAWorkoutGraphViewController
...
- (void)viewWillLayoutSubviews
{
    ...
    if (UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPad) {
        ...
        self.graphViewHeight.constant = 442;
    }
    [self adjustGraphView];
}
...
- (void)setContentsWithDate:(NSDate *)date workoutIndex:(NSInteger )index
{
    ...
    if (data.count > 0 && dataPoints.count > 0){
        ...
        if (!self.isLandscape){
            ...
            WorkoutInfoEntity *workout = [data objectAtIndex:index];
            dataPointIndex = [self getDataPointsForWorkout:workout];
            
            if ([self.delegate conformsToProtocol:@protocol(SFAWorkoutGraphViewControllerDelegate)] &&
            ...
            for (WorkoutInfoEntity *workout in data){
        ...
        [self.graph workoutPortraitLabelsWithBarWidth:self.maxBarWidth barSpace:self.barSpace labelArray:tempGraphLabels numberOfDataPoints:(endingIndex-startingIndex)];
    ...
    if ([self.delegate conformsToProtocol:@protocol(SFAWorkoutGraphViewControllerDelegate)] && [self.delegate respondsToSelector:@selector(graphViewController:didChangeHeartRate:)]){
...
- (NSArray *)getDataPointsForWorkout:(WorkoutInfoEntity *)workout
{
    ...
    if ([self.delegate conformsToProtocol:@protocol(SFAWorkoutGraphViewControllerDelegate)] && [self.delegate respondsToSelector:@selector(graphViewController:didChangeWorkoutEndTime:)]){
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // Values
    if (self.isLandscape && [self.delegate conformsToProtocol:@protocol(SFAWorkoutGraphViewControllerDelegate)]
