//
//  SFADashboardViewController+View.m
//  SalutronFitnessApp
//
...
@implementation SFADashboardViewController (Utilities)
...
- (void)showTryAgainViewWithTarget:(id)target cancelSelector:(SEL)cancelSelector tryAgainSelector:(SEL)tryAgainSelector
{
    ...
    //if ([SFAUserDefaultsManager sharedManager].watchModel == WatchModel_R450) {
    cancel.frame = CGRectMake(0, reminderLabel.frame.origin.y+reminderLabel.frame.size.height, (viewContent.frame.size.width/2), 40);
    //}
    //else{
    ...
    tryAgain.frame = CGRectMake(cancel.frame.origin.x+cancel.frame.size.width, cancel.frame.origin.y, cancel.frame.size.width, cancel.frame.size.height);
    ...
    //cancel.backgroundColor = [UIColor yellowColor];
    //tryAgain.backgroundColor = [UIColor greenColor];
