...
@interface SFASalutronCModelSync() <SalutronSDKDelegate, SFAHealthKitManagerDelegate, UIAlertViewDelegate>
...
//@property (strong, nonatomic) TimeDate              *timeDateNow;
...
@property (readwrite, nonatomic) BOOL watchConnected;

@end

@implementation SFASalutronCModelSync
...
- (void)syncDataWithWatchModel:(WatchModel)watchmodel andStatus:(Status)status{
    ...
    [self didConnectAndSetupDeviceWithStatus:status];
}
...
- (void)updateDateTime
{
    ...
    if(self.isUpdateTimeAndDate) {
        ...
        //self.timeDateNow = timeDateForWatch;
...
- (void)_updateCalibrationData
{
    ...
    if(self.calibrationType < 5) {
        ...
        //calibrationType = 3 - auto el
        //disable auto el - salutron's request
        if (self.calibrationType == 3) {
            self.calibrationType++;
        }
        if(self.calibrationType == 0 || self.calibrationType == 1) {
...
- (void)didDiscoverDevice:(int)numDevice withStatus:(Status)status
{
    ...
    if (self.initialSync) {
        if ([self.delegate respondsToSelector:@selector(didDeviceDiscovered:withStatus:)]) {
            [self.delegate didDeviceDiscovered:numDevice withStatus:status];
        }
    }
    else if(!self.watchConnected){
...
- (void)didConnectAndSetupDeviceWithStatus:(Status)status
{
    if (self.initialSync) {
        if([self.delegate conformsToProtocol:@protocol(SFASalutronSyncDelegate)] &&
           [self.delegate respondsToSelector:@selector(didDeviceConnectedWithStatus:)])
            [self.delegate didDeviceConnectedWithStatus:status];
    }
    else{
...
- (void)didRetrieveConnectedDevice:(int)numDevice withStatus:(Status)status
{
    DDLogInfo(@"\n---------------> STATUS: %@\n", Status_toString[status]);
    if (self.initialSync) {
        [self.delegate didDeviceRetrieved:numDevice withStatus:status];
    }
    else{
        _retrievingDevice = NO;
        DeviceDetail *deviceDetail = nil;
...
- (void)didGetWorkoutDatabase:(NSArray *)workoutDatabase withStatus:(Status)status
{
    ...
    DDLogError(@"workoutDatabase count = %lu", (unsigned long)workoutDatabase.count);
...
- (void)didGetCurrentTimeAndDate:(TimeDate *)timeDate withStatus:(Status)status
{
        ...
        if ([self.userDefaults boolForKey:PROMPT_CHANGE_SETTINGS]) {
            ...
            if(savedTimeDate == nil ||
               ...
               savedTimeDate.dateFormat != timeDate.dateFormat) {
                ...
                // Alert Here.
                // saveUserProfile();
                if ([self.userDefaults boolForKey:AUTO_SYNC_TIME]) {
    ...
    self.updateTimeAndDate = [self.userDefaults boolForKey:AUTO_SYNC_TIME];
    [self performSelector:@selector(updateDateTime) withObject:nil afterDelay:SELECTOR_DELAY];
...
- (void)didUpdateTimeAndDateWithStatus:(Status)status
{
    ...
    //self.watchTimeDate = self.timeDateNow;
    self.updateTimeAndDate = NO;
#warning revisit - causes try again pop up when user selects watch settings after sync on dashboard
...
- (void)startSyncWithWatchModel:(WatchModel)watchModel withDeviceIndex:(int)deviceIndex
{
    ...
    self.watchModel = watchModel;
    self.salutronSDK.delegate = self;
    [self connectToDeviceWithIndex:deviceIndex];
    //[self checkConnectedDevice];
...
- (void)connectToDeviceWithIndex:(int)deviceIndex{
    DDLogInfo(@"\n---------------> DEVICE INDEX : %d\n", deviceIndex);
    //int numDevice = 0;
    BOOL invalidDevice = NO;
    self.discoveredDeviceIndex = deviceIndex;
    //if (numDevice > 0)
    //{
     //   if(status == UPDATE) {
            ...
            if (deviceUUID) {
                for (NSInteger i = 0; i < 1; i ++) {
                    DeviceDetail *deviceDetail = nil;
                    Status s = [self.salutronSDK getDeviceDetail:deviceIndex with:&deviceDetail];
                    NSString *deviceId = deviceDetail.deviceID.description;
                    DDLogInfo(@"deviceUUID: %@\ndeviceDetail: %@", deviceUUID, deviceDetail);
                    if ([deviceId isEqual:WatchModel_R450_DeviceId] || ![SFAWatch isDeviceId:deviceId SameWithWatchModel:self.watchModel]){
                        invalidDevice = YES;
...
- (Status)retrieveConnectedDevices{
    ...
    self.salutronSDK.delegate = self;
   // [self.salutronSDK clearDiscoveredDevice];
    Status s = [self.salutronSDK retrieveConnectedDevice];
    return s;
}
...
- (Status)discoverDevicesWithTimeout:(int)timeout{
    ...
    self.salutronSDK.delegate = self;
    //[self.salutronSDK clearDiscoveredDevice];
    Status s = [self.salutronSDK discoverDevice:timeout];
    return s;
}

- (Status)getDeviceDetailAt:(int)index with:(DeviceDetail **)deviceDetails{
    DeviceDetail *deviceDetail = nil;
    Status s = [self.salutronSDK getDeviceDetail:index with:&deviceDetail];
...
- (Status)connectDeviceAt:(int)index{
    ...
    self.salutronSDK.delegate = self;
    self.watchConnected = NO;
    Status s = [self.salutronSDK connectDevice:index];
    return s;
}
