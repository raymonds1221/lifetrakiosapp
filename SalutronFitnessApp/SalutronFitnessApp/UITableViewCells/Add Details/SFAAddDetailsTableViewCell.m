...
@interface SFAAddDetailsTableViewCell () <UIPickerViewDataSource, UIPickerViewDelegate>
...
@implementation SFAAddDetailsTableViewCell
...
- (void)setWeightContent
{
    ...
    self.textField.placeholder = self.salutronUserProfile.unit == IMPERIAL ? LS_WEIGHT_WITH_UNIT_IMP : LS_WEIGHT_WITH_UNIT_MET;
    ...
    NSString *unit = self.salutronUserProfile.unit == IMPERIAL ? @" lbs" : @" kg";
    ...
    self.textField.text                         = [NSString stringWithFormat:@"%@%@", self.textField.text, unit];
...
- (void)setHeightContent
{
    ...
    self.textField.placeholder = self.salutronUserProfile.unit == IMPERIAL ? LS_HEIGHT_WITH_UNIT_IMP : LS_HEIGHT_WITH_UNIT_MET;
    ...
    NSString *unit      = self.salutronUserProfile.unit == IMPERIAL ? @"" : @" cm";
    self.textField.text                         = [NSString stringWithFormat:@"%@%@", self.textField.text, unit];
...
- (void)setBirthDateContent
{
    ...
    if (_timeDate.dateFormat == 0) {
        ...
        self.textField.placeholder                  = LS_BIRTHDAY_WITH_UNIT_DMY;
    }
    else {
        _birthDateString = [NSString stringWithFormat:@"%i-%i-%i",
        ...
        self.textField.placeholder                  = LS_BIRTHDAY_WITH_UNIT_MDY;
    }
...
- (void)_textFieldStartEditing:(id)sender
{
    ...
    self.cellSeparator.backgroundColor  = LIFETRAK_COLOR;
    self.labelTitle.textColor           = LIFETRAK_COLOR;
    self.labelTitle.hidden              = NO;
}

- (void)_textFieldReturned:(id)sender
{
    self.cellSeparator.backgroundColor  = LIFETRAK_COLOR_INACTIVE;
    self.labelTitle.textColor           = [UIColor lightGrayColor];//LIFETRAK_COLOR_INACTIVE;
    //self.labelTitle.hidden              = self.textField.text.length == 0 ? NO : YES;
    //SalutronUserProfile *_userProfile   = [SalutronUserProfile getData];
    TimeDate *_timeDate                 = [TimeDate getData];
    NSDate *_date;
    ...
    switch (self.textField.tag) {
        case ProfileTypeWeight:
        {
            ...
            _oldValue           = [_oldValue stringByReplacingOccurrencesOfString:unit withString:@""];
